...
def get_pairs(quote_asset='XCH', exclude_pairs=[], max_pairs=12, from_time=None):
    ...
    for currency in quote_assets:
        if len(top_pairs) < max_pairs:
            ...
            for currency_pair in currency_pairs:
                ...
                if currency_pair['pair'] == 'XCH/VIA': # XCP/BTC always in first
                    top_pairs.insert(0, top_pair)
                else:
                    top_pairs.append(top_pair)
                all_assets += [currency_pair['base_asset'], currency_pair['quote_asset']]

    if ('VIA' in quote_assets) and ('XCH/VIA' not in [p['base_asset'] + '/' + p['quote_asset'] for p in top_pairs]):
        top_pairs.insert(0, {
            'base_asset': 'XCH',
            'quote_asset': 'VIA'
        })
