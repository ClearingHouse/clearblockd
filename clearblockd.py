...
if __name__ == '__main__':
    ...
    counterwallet_config_path = os.path.join('/home/xch/clearwallet/clearwallet.conf.json')
    if os.path.exists(counterwallet_config_path):
        logging.info("Loading clearwallet config at '%s'" % counterwallet_config_path)
        with open(counterwallet_config_path) as f:
            config.COUNTERWALLET_CONFIG_JSON = f.read()
    else:
        logging.warn("Clearwallet config does not exist at '%s'" % counterwallet_config_path)
        config.COUNTERWALLET_CONFIG_JSON = '{}'
    try:
        config.COUNTERWALLET_CONFIG = json.loads(config.COUNTERWALLET_CONFIG_JSON)
    except Exception, e:
        logging.error("Exception loading clearwallet config: %s" % e)
